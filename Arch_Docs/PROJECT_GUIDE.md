...
graph TD
    %% --- 入口层 ---
    User((User)) -->|Request (REST/SSE)| API[FastAPI Gateway<br/>(Auth/Tenant/Middleware)]
    API --> MasterGraph_Start([Master Graph Start])

    %% --- 1. 核心路由与技能层 (Kernel: Skill System) ---
    subgraph Kernel_Skills [Kernel: Skill Routing & Evolution]
        style Kernel_Skills fill:#e1f5fe,stroke:#01579b
        
        MasterGraph_Start --> SkillRouter{Skill Router<br/>(Vector Search)}
        
        SkillRouter -->|Match L1/L2<br/>(Specific/Domain)| SkillExecutor[Skill Executor<br/>(Fast Track)]
        
        SkillRouter -->|Match L3/None<br/>(Meta/Fail)| EvolverCheck{Need Evolution?}
        
        EvolverCheck -->|Yes| SkillEvolver[Skill Evolver Agent<br/>(Gap Analysis -> Draft -> Persist)]
        SkillEvolver -->|New Skill Ready| SkillExecutor
        
        EvolverCheck -->|No/Complex| GlobalPlanner[Global Planner<br/>(Task Decomposition)]
        
        SkillExecutor --> FinalAnswer
    end

    %% --- 2. 编排与分发层 (Kernel: Orchestration) ---
    subgraph Kernel_Orch [Kernel: Orchestration]
        style Kernel_Orch fill:#f3e5f5,stroke:#4a148c
        
        GlobalPlanner --> ApprovalCheck{Requires<br/>Approval?}
        
        ApprovalCheck -->|Yes| HumanInterrupt[Human Approval<br/>(Interrupt Point)]
        HumanInterrupt -->|Approved| Dispatcher[Dispatcher Node]
        ApprovalCheck -->|No| Dispatcher
        
        Dispatcher -->|Chitchat| ChitchatNode[Chitchat Node]
        Dispatcher -->|Task A| Plugin_Voyager[Plugin: Voyager]
        Dispatcher -->|Task B| Plugin_Tutorial[Plugin: Tutorial]
        Dispatcher -->|Task C| Plugin_FTA[Plugin: FTA]
        
        ChitchatNode --> TaskResult
    end

    %% --- 3. 插件运行层 (Plugin Runtime) ---
    subgraph Plugin_Runtime [Plugin Runtime Environment]
        style Plugin_Runtime fill:#fff3e0,stroke:#e65100
        
        Plugin_Voyager & Plugin_Tutorial & Plugin_FTA --> LocalPlanner[Local Planner]
        
        LocalPlanner --> ToolRouter{Tool Router}
        
        ToolRouter -->|Internal| LocalTools[Local Tools<br/>(Python Functions)]
        ToolRouter -->|External| MCPServer[MCP Manager<br/>(Model Context Protocol)]
        
        LocalTools & MCPServer --> Observation[Observation]
        
        Observation --> Critic{Critic/Reflect}
        
        Critic -->|Retry/Refine| LocalPlanner
        Critic -->|Success| TaskResult([Task Result])
    end

    %% --- 4. 循环与输出 ---
    TaskResult -->|Next Task| Dispatcher
    TaskResult -->|All Tasks Done| FinalAnswer[Final Answer]
    
    FinalAnswer -->|Response Stream| User

    %% --- 数据存储与外部依赖 (Infrastructure) ---
    Qdrant[(Qdrant<br/>Vectors/Memory)]
    Postgres[(PostgreSQL<br/>Skills/State)]
    ExtServices[External Services<br/>(Files/Git/DB/Web)]

    SkillRouter -.->|Retrieve| Qdrant
    SkillEvolver -.->|Save| Postgres
    SkillEvolver -.->|Index| Qdrant
    
    MCPServer -.->|Connect| ExtServices
    MasterGraph_Start -.->|Load Checkpoint| Postgres
```
